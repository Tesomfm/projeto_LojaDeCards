...
def criar(dados: CriarCarta, db: Session = Depends(get_db), token: dict = Depends(validar_token)):
    return crud_cartas.criar_carta(db, dados)

@rotas.put("/{carta_id}", response_model=CartaResponse)
def substituir(carta_id: int, dados: CriarCarta, db: Session = Depends(get_db), token: dict = Depends(validar_token)):
...
def atualizar(carta_id: int, dados: CartaUpdate, db: Session = Depends(get_db), token: dict = Depends(validar_token)):
...
def deletar(carta_id: int, db: Session = Depends(get_db), token: dict = Depends(validar_token)):

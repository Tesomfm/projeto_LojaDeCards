...
# Configuração do validador de Token do FastAPI
security = HTTPBearer()
...
@rotas.post("/", response_model=CartaResponse, status_code=201)
def criar(dados: CriarCarta, db: Session = Depends(get_db), token_data: dict = Depends(verificar_token)):
    return crud_cartas.criar_carta(db, dados)

@rotas.put("/{carta_id}", response_model=CartaResponse)
def substituir(carta_id: int, dados: CriarCarta, db: Session = Depends(get_db), token_data: dict = Depends(verificar_token)):
    carta = crud_cartas.substituir_carta(db, carta_id, dados)
    if not carta:
        raise HTTPException(status_code=404, detail="Carta não encontrada.")
    return carta
...
@rotas.patch("/{carta_id}", response_model=CartaResponse)
def atualizar(carta_id: int, dados: CartaUpdate, db: Session = Depends(get_db), token_data: dict = Depends(verificar_token)):
    carta = crud_cartas.atualizar_carta(db, carta_id, dados)
    if not carta:
        raise HTTPException(status_code=404, detail="Carta não encontrada.")
    return carta
...
@rotas.delete("/{carta_id}", status_code=204)
def deletar(carta_id: int, db: Session = Depends(get_db), token_data: dict = Depends(verificar_token)):
    crud_cartas.deletar_carta(db, carta_id)